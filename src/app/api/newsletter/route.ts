...
/**
 * Email-only install gate: this endpoint accepts the email, sends the welcome
 * email containing a tokenized install URL, and stops. It does NOT set the
 * install-gate cookie. The cookie is only stamped by /api/download when the
 * user clicks the email link with a valid token (so the user must actually
 * receive and act on the email; submitting the form alone is not enough to
 * unlock the DMG, even from the same browser).
 */
