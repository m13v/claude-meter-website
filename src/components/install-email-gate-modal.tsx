...
function getCapturedInstallEmail(): string {
...
interface Props {
  ...
  onClose: () => void;
  /**
   * Optional, retained for backward compat. The email-only flow does NOT
   * navigate anywhere after submit; it shows a "check your email" success
   * state and the user must click the install link in their inbox. This
   * callback fires when the user dismisses the success state, in case a
   * caller wants to know the gate completed (e.g. to refresh UI). Most
...
export function InstallEmailGateModal({ open, onClose, onComplete, section, destination, text }: Props) {
  ...
  // Pre-fill the email field from localStorage so returning users can resend
  // the install link with one click.
  useEffect(() => {
    if (open) {
      setEmail((cur) => cur || getCapturedInstallEmail());
      setSubmitted(false);
      setError("");
    }
  }, [open]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 5xx fall through: don't block the success message; Resend may still
      // deliver, and even if it doesn't the user can retry.
      try {
            ...
            {!submitted ? (
              <div className="p-7">
                ...
                <form onSubmit={onSubmit} className="space-y-3">
                  ...
                  >
                    {submitting ? "Sending\u2026" : "Email me the install link"}
                  </button>
                </form>
                <p className="mt-4 text-xs text-zinc-500">
                  The install link is delivered by email only. Check your inbox after submit.
                </p>
              </div>
            ) : (
              <div className="p-7">
                <div className="mb-5 flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 grid h-8 w-8 place-items-center rounded-full bg-emerald-500/15 text-emerald-400">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm leading-relaxed text-zinc-400">
                        We sent the install link to{" "}
                        <span className="font-medium text-white">{submittedEmail}</span>.
                        Open it on whichever device you want to install on.
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="rounded-xl border border-white/10 bg-black/30 p-4 text-xs leading-relaxed text-zinc-400">
                  Email usually arrives within seconds. If you don&rsquo;t see it after
                  a minute, check spam or promotions, or submit again with a different
                  address.
                </div>
                ...
                <button
                  type="button"
                  onClick={handleClose}
                  className="mt-5 w-full rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10"
                >
                  Got it
                </button>
              </div>
